...
from lib.utils.utils import save_checkpoint, save_state_dict
...
def main():
    ...
    data_loaders = {
        client_id: create_data_generator(client_id, rank) for client_id in cfg.FED.CLIENT_IDS
    }

    for epoch in range(1, cfg.FED.EPOCHS + 1):
        logger.info(f"#### Federated Learning Round {epoch}/{cfg.FED.EPOCHS + 1} ####")
        # Train each model on different client datasets
        client_models = []
        for client_id in cfg.FED.CLIENT_IDS:
            clinet_model = train_client_model(global_model, epoch, data_loaders[client_id], cfg, logger)
            client_models.append(clinet_model)

        # Update global model
        global_model = fedavg(global_model, client_models)

def train_client_model(model, epoch, data_loader, cfg, logger):
    global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1

    rank = global_rank
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # Train End-to-End model on each client dataset
    # define loss function (criterion) and optimizer
    # TODO: You may need to modify get_optimizer to accept different learning rates for different clients
